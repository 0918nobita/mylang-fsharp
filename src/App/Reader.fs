...
module Reader =
    ...
    let local (f: 'Env1 -> 'Env2) (Reader action: Reader<'Env2, 'T>) : Reader<'Env1, 'T> = Reader(f >> action)

type ReaderBuilder() =
    member inline _.Bind(x: Reader<'Env, 'T>, [<InlineIfLambda>] f: 'T -> Reader<'Env, 'U>) : Reader<'Env, 'U> =
        Reader.bind f x

    member inline this.Combine
        (
            reader1: Reader<'Env, unit>,
            [<InlineIfLambda>] reader2: unit -> Reader<'Env, 'T>
        ) : Reader<'Env, 'T> =
        this.Bind(reader1, reader2)

    member inline _.Delay([<InlineIfLambda>] delayedReader: unit -> Reader<'Env, 'T>) : unit -> Reader<'Env, 'T> =
        delayedReader

    member _.Return(x: 'T) : Reader<'Env, 'T> = Reader(fun _ -> x)

    member inline _.ReturnFrom(reader: Reader<'Env, 'T>) : Reader<'Env, 'T> = reader

    member inline _.Run([<InlineIfLambda>] delayedReader: unit -> Reader<'Env, 'T>) : Reader<'Env, 'T> =
        delayedReader ()
